...
<!-- Mobile overlay -->
{#if !collapsed}
  <div class="fixed inset-0 bg-black/30 z-20 lg:hidden" on:click={() => (collapsed = true)}></div>
{/if}
...
>
  ...
  <div class="flex items-center gap-3 px-5 py-6 border-b border-white/5">
    <div class="w-8 h-8 rounded-full bg-gold flex items-center justify-center text-navy font-serif font-bold text-sm shrink-0">
      B
    </div>
    <div>
      <div class="font-serif text-white font-bold text-sm">Brisa Foundation</div>
      <div class="text-white/30 text-xs">Admin Panel</div>
    </div>
  </div>
  ...
  <!-- Bottom section -->
  <div class="px-3 pb-5 pt-3 border-t border-white/5 space-y-2">
    <a
      href="/"
      target="_blank"
      class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-white/40 hover:text-white/70 transition-colors"
    >
    ...
    <a
      ...
      class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-red-400/70 hover:text-red-400 transition-colors"
    >
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    {#if userEmail}
      <div class="px-3 pt-2">
        <p class="text-white/20 text-xs truncate">{userEmail}</p>
      </div>
    {/if}
  </div>
</aside>
...
<button
  class="fixed bottom-6 left-6 z-50 lg:hidden w-12 h-12 bg-navy rounded-full shadow-xl flex items-center justify-center text-white"
